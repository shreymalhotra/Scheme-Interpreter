...
'TheFinalProject
; expect thefinalproject

'(v . 2)
; expect (v . 2)

(lambda (x y) (+ x (/ x y)))
; expect (lambda (x y) (+ x (/ x y)))

;;; Q 08

(define (area-of-square x) (* x x))
; expect area-of-square
(area-of-square 10)
; 100
...
(define inner True)
; expect inner
(if inner 'beginner 'expert)
; expect beginner
(let ((inner 'beginner)) (and 'i 'am 'an expert))
; expect Error: unknown identifier: expert
...
(let ((x 'greetings)
      (y 'folks))
      x 2 3 4 (list x y))
; expect (greetings folks)

;;; Q 17

(define a 10)
; expect a
(define lambda-form (lambda (b) (* a b)))
; expect lambda-form
(define (new f b) (define a 100) (f b))
; expect new
(new lambda-form 2)
; expect 20
...
;Question 11
(define ('f 'x) (* x x))
; expect Error: not valid symbols

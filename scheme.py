...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        eval_var = scheme_eval(expressions.first,env)
        if scheme_true(eval_var):
            return eval_var
...
def check_formals(formals):
    ...
    no_duplicates_list = []
    while formals is not nil:
        if scheme_symbolp(formals.first) and formals.first not in no_duplicates_list:
            no_duplicates_list.append(formals.first)
        else: 
            raise SchemeError
        formals = formals.second
    # END Question 11B
